    ...
    return (
        <div className="min-h-screen bg-background text-foreground transition-colors duration-200">
            ...
            <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-card border border-border p-4 rounded-3xl backdrop-blur-md mb-6">
                ...
                {filteredOrders.length === 0 ? (
                    <div className="bg-card border border-border rounded-xl p-8 sm:p-12 text-center max-w-lg mx-auto my-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        {filteredOrders.map((order) => (
                            <div
                                key={order.id}
                                className="bg-card border border-border rounded-xl p-5 sm:p-6 flex flex-col justify-between shadow-sm hover:border-white/10 transition-all"
                            >
                                <div>
...
function StarRating({ value, onChange, label }: { value: number; onChange: (v: number) => void; label: string }) {
    return (
        <div className="flex items-center gap-1">
            <span className="text-xs font-semibold text-muted-foreground mr-1">{label}</span>
            {[1, 2, 3, 4, 5].map((star) => (
                <button
                    key={star}
                    type="button"
...
function ReviewForm({ orderId, onReviewSubmitted }: ReviewFormProps) {
    ...
    return (
        <form onSubmit={handleSubmit} className="mb-4 p-4 bg-background border border-border rounded-2xl space-y-3">
