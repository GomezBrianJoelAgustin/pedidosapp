...
export default function CashierIndex({ awaitingApproval, pendingAssignment, pendingCashPayment, recentOrders, rejectedOrders, deliveryUsers }: PageProps) {
    ...
    const statusBadge: Record<string, { label: string; className: string }> = {
        ...
        out_for_delivery: { label: 'Viajando al Destino', className: 'bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20' },
        at_location: { label: 'El Cadete Está Afuera', className: 'bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20' },
                ...
                <div className="fixed inset-0 z-50 bg-slate-900/40 dark:bg-black/60 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4">
                    <div className="bg-white dark:bg-[#0f0f11] border-t sm:border border-slate-200 dark:border-white/10 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-sm p-6 space-y-5">
                        <div className="flex items-center gap-3 text-amber-600 dark:text-amber-400">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Modalidad de Pago</h3>
                                <p className="text-xs text-slate-500 dark:text-slate-400">Pedido #{selectedOrder.id}</p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleUpdatePayment} className="space-y-4">
                            <div className="space-y-2">
                                <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1.5">Modalidad</label>
                                <label className={`flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all ${paymentForm.payment_status === 'paid' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'border-slate-200 dark:border-white/10 hover:border-slate-300'}`}>
                                    <input
                                        type="radio"
                                        name="payment_status"
                                        value="paid"
                                    ...
                                    />
                                    <div>
                                        <p className="text-sm font-bold text-slate-800 dark:text-white">Paga Ahora</p>
                                        <p className="text-xs text-slate-500 dark:text-slate-400">El pago ya fue recibido en el momento del pedido.</p>
                                    </div>
                                </label>
                                <label className={`flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all ${paymentForm.payment_status === 'pending_payment' ? 'border-amber-500 bg-amber-50 dark:bg-amber-500/10' : 'border-slate-200 dark:border-white/10 hover:border-slate-300'}`}>
                                    <input
                                        type="radio"
                                        name="payment_status"
                                    ...
                                    />
                                    <div>
                                        <p className="text-sm font-bold text-slate-800 dark:text-white">Paga Después</p>
                                        <p className="text-xs text-slate-500 dark:text-slate-400">El pago se realizará al retirar/recibir el pedido.</p>
                                    </div>
                                </label>
                            </div>
